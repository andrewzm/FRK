...
<<eval=TRUE>>=
## Prediction (BAU) grid
isea3h_sp_poldf <- auto_BAUs(manifold=sphere(),res=6,data=NULL)
@
...
<<eval=TRUE>>=
S <- SRE.fit(S,n_EM = 3,tol = 1e-5,print_lik=T)
@

<<eval=TRUE>>=
isea3h_sp_poldf <- SRE.predict(S,pred_locs = isea3h_sp_poldf,use_centroid = T)
@
