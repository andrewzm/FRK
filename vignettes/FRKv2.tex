...
% This vignette serves to demonstrate the use of the \proglang{R} package \pkg{FRK} for analysing non-Gaussian spatial and spatio-temporal data. 
...
\hlkwd{library}\hlstd{(}\hlstr{"reshape2"}\hlstd{)}  \hlcom{# for easy data manipulation}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{m} \hlkwb{<-} \hlnum{250}                                                   \hlcom{# Sample size}
...
\hlstd{zdf}\hlopt{$}\hlstd{Y} \hlkwb{<-} \hlnum{3} \hlopt{+} \hlkwd{sin}\hlstd{(}\hlnum{7} \hlopt{*} \hlstd{zdf}\hlopt{$}\hlstd{x)} \hlopt{+} \hlkwd{cos}\hlstd{(}\hlnum{9} \hlopt{*} \hlstd{zdf}\hlopt{$}\hlstd{y)}               \hlcom{# Latent process}
\hlstd{zdf}\hlopt{$}\hlstd{z} \hlkwb{<-} \hlkwd{rpois}\hlstd{(m,} \hlkwc{lambda} \hlstd{=} \hlkwd{exp}\hlstd{(zdf}\hlopt{$}\hlstd{Y))}                     \hlcom{# Simulate data}
...
\caption{Americium soil data and blocking scheme.\label{fig:Am_data}}\label{fig:unnamed-chunk-8}
\end{figure}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{BAUs} \hlkwb{<-} \hlkwd{auto_BAUs}\hlstd{(}\hlkwc{manifold} \hlstd{=} \hlkwd{plane}\hlstd{(),}
                  \hlkwc{type} \hlstd{=} \hlstr{"grid"}\hlstd{,}
                  \hlkwc{data} \hlstd{= Am_data,}
                  \hlkwc{nonconvex_hull} \hlstd{=} \hlnum{FALSE}\hlstd{)}

\hlcom{## Add covariates to the BAUs}
\hlstd{d_cutoff} \hlkwb{<-} \hlnum{30.48}
\hlstd{d_BAU} \hlkwb{<-} \hlkwd{distR}\hlstd{(}\hlkwd{coordinates}\hlstd{(BAUs), GZ_df)}
\hlstd{BAUs}\hlopt{$}\hlstd{x1} \hlkwb{<-} \hlkwd{as.numeric}\hlstd{(d_BAU} \hlopt{<} \hlstd{d_cutoff)}
\hlstd{BAUs}\hlopt{$}\hlstd{x2} \hlkwb{<-} \hlstd{d_BAU} \hlopt{*} \hlstd{BAUs}\hlopt{$}\hlstd{x1}
\hlstd{BAUs}\hlopt{$}\hlstd{x3} \hlkwb{<-} \hlkwd{as.numeric}\hlstd{(d_BAU} \hlopt{>=} \hlstd{d_cutoff)}
\hlstd{BAUs}\hlopt{$}\hlstd{x4} \hlkwb{<-} \hlstd{d_BAU} \hlopt{*} \hlstd{(BAUs}\hlopt{$}\hlstd{x3)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{BAUs}\hlopt{$}\hlstd{fs}     \hlkwb{<-} \hlnum{1}
\hlstd{Am_data}\hlopt{$}\hlstd{std} \hlkwb{<-} \hlnum{1}

\hlstd{S} \hlkwb{<-} \hlkwd{FRK}\hlstd{(}\hlkwc{f} \hlstd{= Am} \hlopt{~ -}\hlnum{1} \hlopt{+} \hlstd{x1} \hlopt{+} \hlstd{x2} \hlopt{+} \hlstd{x3} \hlopt{+} \hlstd{x4,} \hlkwc{data} \hlstd{=} \hlkwd{list}\hlstd{(Am_data),}
         \hlkwc{response} \hlstd{=} \hlstr{"gaussian"}\hlstd{,}
         \hlkwc{link} \hlstd{=} \hlstr{"log"}\hlstd{,}
         \hlkwc{BAUs} \hlstd{= BAUs,}
         \hlkwc{nres} \hlstd{=} \hlnum{2}\hlstd{,}
         \hlkwc{est_error} \hlstd{=} \hlnum{FALSE}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{pred} \hlkwb{<-} \hlkwd{predict}\hlstd{(S,} \hlkwc{type} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"link"}\hlstd{,} \hlstr{"mean"}\hlstd{))}
\hlstd{plot_list} \hlkwb{<-} \hlkwd{plot}\hlstd{(S, pred)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\caption{Americium point predictions.\label{fig:Am_BAU_predictions}}\label{fig:unnamed-chunk-12}
\end{figure}
