...
zdf$Y <- sin(8 * zdf$x) + cos(8 * zdf$y) + 0.5 * rnorm(m)  # Simulate latent process
zdf$z <- rpois(m, lambda = zdf$Y^2)                        # Simulate data
...
S <- FRK(f = z ~ 1,               # Formula to FRK
         ...
         nres = 1,                # Reduce run-time using only 1 basis function resolution
...
<<warning=FALSE,fig.align='center',fig.cap="(Left) Simulated poisson data. (Centre) Prediction of the mean process. (Right) Uncertainty quantification of predictions, specifically the width of the 90\\% posterior predictive interval.\\label{fig:example1}",fig.subcap=c("","")>>=
plot_list <- plot(S, pred, zdf)
ggarrange(plot_list$z + labs(fill = "data"), 
          plot_list$p_mu + labs(fill = "pred."), 
          plot_list$interval90_mu + labs(fill = "pred.\nuncertainty"), 
          nrow = 1, legend = "top") 
@

\section{Example: Log-normal point and block prediction}
