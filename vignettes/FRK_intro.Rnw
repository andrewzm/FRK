...
\end{equation}
\noindent where $\tvec_i \equiv \frac{1}{|A_i|}\int_{A_i} \tvec(\svec) \intd \svec$, $\upsilon_i\equiv \frac{1}{|A_i|}\int_{A_i} \upsilon(\svec) \intd \svec$, and $\xi_i$ is specified later.
...
\end{equation}
where $\{\svec_i: i = 1,\dots,N\}$ are the centroids of the BAUs. This approximation can be used to simplify computations when several, small, BAUs are used.

In \tt{FRK}, we do not explicitly model $\xi(\svec)$. Rather, we assume that $\xi_i = \frac{1}{|A_i|}\int_{A_i} \xi(\svec)\intd\svec$ is normally distributed with mean zero and variance
...
\end{equation}
where $\sigma^2_\xi$ is a parameter that needs to be estimated, $\{\varv_{\xi,1},\dots,\varv_{\xi,N}\}$  represent known heteroscedasticity, and $\{\xi_i: i = 1,\dots,N\}$ are independent. From \eqref{eq:Yi1}, we can write
...
Each $B_j \in D^O$ is either a BAU, or a union of BAUs. We define the measurement process as
\begin{equation}
...
\end{equation}
where
$$ w_{ij} = |A_i|\mathbb{I}(A_i \subset B_j)$$
weight the process averages according to the areas of the BAUs, $\epsilon_j$ is Gaussian measurement error, $\mathbb{I}(\cdot)$ is the indicator function, and $\delta_i$ is spatially-dependent \emph{systematic error} in the measurement. As with the fine-scale variation, we assume that $\{\delta_i: i = 1,\dots,N\}$ are independent with variance
\begin{equation}
\var(\delta_i) = \sigma^2_\delta \varv_{\delta,i},
\end{equation}
where $\sigma^2_\delta$ is a parameter that needs to be estimated, $\{\varv_{\delta,1},\dots,\varv_{\delta,N}\}$  represent known heteroscedasticity, and $\{\delta_i: i = 1,\dots,N\}$ are independent.

We assume that the observations are conditionally independent, when conditioned on $\Yvec$ \emph{and} $\deltab$. Therefore $\{\epsilon_j: j = 1,\dots,m\}$, where $m \equiv |D^O|$ is the number of observations, are independent. If these errors are also identically distributed, then $\var(\epsilon_j), j = 1,\dots,m$ can in principle be estimated from the data (provided neither $\Vmat_\xi$ nor $\Vmat_\delta \equiv \diag(\varv_{\delta,1},\dots,\varv_{\delta,N})$ are also not proportional to the identity matrix ).

Let $\Zvec \equiv (Z_j : j = 1,\dots,m)'$. Then, since each element in $D^O$ is the union of subsets of $D^L$, one can construct a matrix $\Cmat_Z$ using the weights $\{w_{ij}\}$, such that
\begin{equation}
\Zvec = \Cmat_Z\Yvec + \Cmat_Z\deltab +  \epsilonb,
\end{equation}
\noindent where $\epsilonb \equiv (\epsilon_j : j = 1,\dots,m)', \deltab \equiv (\delta_j : j = 1,\dots,m)',$ and $\var(\epsilonb) = \Sigmamat_\epsilon$ is a diagonal covariance matrix. The rows of  the matrix $\Cmat_Z$ sum to 1. It will be convenient, as a consequence of conditional independence, to re-write
\begin{equation}\label{eq:Z_collapsed}
\Zvec = \Tmat_Z\alphab + \Smat_Z\etab + \xib_Z + \deltab_Z + \epsilonb,
\end{equation}
where $\Tmat_Z \equiv \Cmat_Z \Tmat$, $\Smat_Z \equiv \Cmat_Z \Smat$, $\xib_Z \equiv \Cmat_Z \xib$, $\deltab_Z \equiv \Cmat_Z \deltab$, and where $\var(\xib_Z) = \sigma^2_\xi\Vmat_{\xi,Z} \equiv \sigma^2_\xi\Cmat_Z\Vmat_{\xi}\Cmat_Z'$ and $\var(\deltab_Z) = \sigma^2_\delta\Vmat_{\delta,Z} \equiv \sigma^2_\delta\Cmat_Z\Vmat_\delta\Cmat_Z'$. In practice, it is not always possible for each $B_i$ to overlap a BAU exactly. For simplicity, we assume that the observation footprint overlaps a BAU only if the BAU centroid lies within the footprint. Frequently, data is also supplied as point-referenced. In this case each data point is attributed to a specific BAU and it is possible to have multiple observations of the same BAU.


We collect the unknown parameters in the set $\thetab \equiv \{\alphab, \sigma^2_\xi, \sigma^2_\delta, \Kmat\}$; their estimation is the subject of Section \ref{sec:estimation}. Once the parameters in $\thetab$ are estimated, a straightforward inversion may then be used to predict over unknown areas. In \texttt{FRK} we allow the prediction set $D^P$ to be as flexible as $D^O$; specifically, $D^P \subset \{ \cup_{k \in c_k} A_i : c_k \in 2^{\{1,2,\dots,N\}} \setminus \varnothing, k = 1,\dots,N_P \}$, where $N_P$ is the number of prediction areas. We can thus predict both at the individual BAU level or averages over an area spanning multiple BAUs, and these prediction regions may overlap. We provide the prediction equations in Section \ref{sec:prediction}.

\subsection{Parameter estimation using an EM algorithm} \label{sec:estimation}

In all its generality, parameter estimation with the model \ref{sec:SREModel} is problematic due to confounding between $\deltab$ and $\xib$. In \tt{FRK}, the user thus needs to choose where the fine-scale variation sits, either in the observation model (in which case $\sigma^2_\xi$ is fixed to 0) or in the process model (in which case $\sigma^2_\delta$ is fixed to 0). Below we describe the estimation procedure for the latter case; due to symmetry, the equations of the former case can be obtained by simply replacing the subscript $\xi$ with $\delta$. Which case is chosen has a considerable impact on the prediction equations (Section \ref{sec:prediction}).
...
\end{align}
and where $\Dmat_Z^{(l)} \equiv (\sigma^2_\xi)^{(l)} \Vmat_Z + \Sigmamat_\epsilon$.
...
\end{align}
while that for $\sigma_\xi^{2^{(l+1)}}$ requires the solution to
\begin{equation} \label{eq:sigma2d}
\tr((\Sigmamat_{\epsilon} + (\sigma^2_\xi)^{(l+1)}\Vmat_Z)^{-1}\Vmat_Z) = \tr((\Sigmamat_{\epsilon} + (\sigma^2_\xi)^{(l+1)}\Vmat_Z)^{-1}\Vmat_Z(\Sigmamat_{\epsilon} + (\sigma^2_\xi)^{(l+1)}\Vmat_Z)^{-1}\Omegab),
\end{equation}
where
...
\begin{equation}
(\sigma^2_\xi)^{(l+1)} = \frac{1}{c_1} \left(\frac{\tr(\Omegab)}{m} - c_2 \right),
\end{equation}
...
\begin{equation}
\ln [p(]\Zvec \mid \alphab^{(l)}, \Kmat^{(l)}, (\sigma^2_\xi)^{(l)}] = -\frac{m}{2}\ln 2\pi -\frac{1}{2}\ln |\Sigmamat_Z^{(l)}| - \frac{1}{2}(\Zvec - \Tmat_Z\alphab^{(l)})'\Sigmamat_Z^{-1^{(l)}}(\Zvec - \Tmat_Z\alphab^{(l)}),
\end{equation}
where
\begin{equation}
\Sigmamat_Z^{(l)} = \Smat_Z \Kmat^{(l)} \Smat_Z' + \Dmat_Z^{(l)},
\end{equation}
and recall that $\Dmat_Z^{(l)} \equiv (\sigma_\xi^2)^{(l)}\Vmat_Z + \Sigmamat_\epsilon$. Efficient computation of the log-likelihood is facilitated through the use of the Sherman-Woodbury matrix identity and a matrix-determinant lemma \citep[e.g.,][]{Cressie_2008}. Specifically, the operations
\begin{align}
...
They key object of prediction is to estimate $Y(\svec)$ over $D^P$. Consider the process $Y_P(B_j)$, which, similar to the observations, is constructed using the BAUs $\{A_i: i = 1,\dots,N\}$:

\begin{equation}
Y_{P,j} \equiv Y_{P}(B_j) = \left(\frac{\sum_{i =1}^N Y_i w_{ij}}{\sum_{i=1}^N w_{ij}}\right); \quad B_j \in D^P.
\end{equation}
Let $\Yvec_P \equiv (Y_{P,j} : j = 1,\dots,N_P)'$, where $N_P$ is the number of prediction areas and is also equal to $|D^P|$. Then, since each element in $D^P$ is the union of subsets of $D^L$, one can construct a matrix $\Cmat_P$, the rows of which sum to one, such that
\begin{equation}
\Yvec_P = \Cmat_P\Yvec = \Tmat_P\alphab + \Smat_P\etab + \xib_P,
\end{equation}
where $\Tmat_P \equiv \Cmat_P \Tmat$, $\Smat_P \equiv \Cmat_P \Smat$, $\xib_P \equiv \Cmat_P \xib$ and $\var(\xib_P) = \sigma^2_\xi\Vmat_{\xi,P} \equiv \sigma^2_\xi\Cmat_P\Vmat_\xi\Cmat_P'$.  As with the observations, these prediction regions may overlap. In practice, it is not always possible for each $B_i$ to overlap a BAU directly. In this case, we assume that the prediction region overlaps a BAU only if the BAU centroid lies within the prediction area.

Let $l^*$ denote the EM iteration number at which convergence was reached. The final estimates are then $\widehat\muvec_\eta = \muvec_\eta^{(l^*)}, \widehat\Sigmamat_\eta = \Sigmamat_\eta^{(l^*)}, \widehat\alpha = \alpha^{(l^*)}, \widehat\Kmat = \Kmat^{(l^*)}, \widehat\sigma^2_\xi = (\sigma^2_\xi)^{(l^*)},$ and $\widehat\sigma^2_\delta = (\sigma^2_\delta)^{(l^*)}$.

{\bf Case 1: $\sigma^2_\xi = 0$ (Default).} The prediction vector and covariance matrix corresponding to $\Yvec$ when $\sigma^2_\xi = 0$ are
\begin{align}
\widehat\Yvec \equiv \E(\Yvec \mid \Zvec) &= \Tmat\widehat\alphab + \Smat\widehat\muvec_\eta, \label{eq:smooth1}\\
\Sigmamat_{Y \mid Z} \equiv \var(\Yvec\mid \Zvec) &= \Smat \widehat\Sigmamat_\eta\Smat'. \label{eq:smooth2}
\end{align}

{\bf Case 2: $\sigma^2_\delta = 0$.} In order to predict $\Yvec_P$ when $\sigma^2_\delta = 0$, we also need to predict $\xib_P$, which is not available at this stage. Since we generally wish to predict at locations other than where we have observations, this quantity is rarely equal to $\xib_Z$; this is why $\xib_Z$ was not estimated in Section \ref{sec:estimation}.

To cater for arbitrary observation and prediction support, we predict $\Yvec_P$ by first carrying out prediction over $\Yvec$, that is, at the BAU level, and then transforming linearly through $\Cmat_P$. Let $\Wvec \equiv (\etab',\xib')'$ and $\Pimat \equiv (\Smat,\Imat)$. Then \eqref{eq:SRE_Y} can be re-written as $\Yvec = \Tmat\alphab + \Pimat\Wvec$ and
\begin{align}
...
\end{align}
and the block diagonal matrix $\Lambdamat \equiv \textrm{bdiag}(\widehat\Kmat,\widehat\sigma^{2}_\xi\Vmat_{\xi,P})$, where $\textrm{bdiag}(\cdot)$ returns a block diagonal matrix of its arguments.



For both Cases 1 and 2 it follows that $\E(\Yvec_P \mid \Zvec) = \Cmat_P\widehat\Yvec$ and
...
\noindent {\bf Step 6:} Finally, we predict at all the BAUs with the fitted model. This is done using the function \tt{SRE.predict}. The argument \tt{use\_centroid} dictates whether we invoke the approximation \eqref{eq:Sapprox} or not, see \tt{help(SRE.predict)} for details. The argument \tt{obs\_fs} dictates whether we attribute the fine-scale variation to the process model or the observation model (in which case it takes the role of systematicerror). Below, we allocate it to the process model.
...
isea3h_sp_poldf <- SRE.predict(SRE_model = S,          # SRE model
                               use_centroid = TRUE,    # use centroid as point reference
                               obs_fs = TRUE)          # fs variation is in the observation model
@
...
<<echo=FALSE,fig.width=7,fig.height=3.5,out.width="\\linewidth",fig.pos="t!",fig.cap="Prediction of $\\Yvec_P$ in ppm following FRK on the \\tt{AIRS} data.\\label{fig:AIRSresults2}",fig.align="centre">>=
g2 <- (EmptyTheme() +
           geom_polygon(data=X,
...
<<echo=FALSE,fig.keep=TRUE,fig.width=7,fig.height=3.5,out.width="\\linewidth",fig.pos="t!",fig.cap="Prediction standard error of $\\Yvec_P$ in ppm following FRK on the \\tt{AIRS} data.\\label{fig:AIRSresults3}",fig.align="centre">>=
...
<<SRE,results='hide',cache=FALSE>>=
 ...
 S <- SRE.fit(SRE_model = S,    # estimate parameters in the SRE model S
             n_EM = 4,        # maximum no. of EM iterations
...
G_spatial <- auto_basis(manifold = sphere(),      # functions on sphere
                        ...
                        nres = 4,                 # use three DGGRID resolutions
                        prune=15,                 # prune basis functions
                        type = "bisquare",        # bisquare basis functions
...
<<echo=FALSE, fig.keep=TRUE,fig.cap="Prediction of $\\Yvec_P$ in ppm on 04, 08, and 12 May 2003 obtained with \\tt{FRK} on the \\tt{AIRS} data. \\label{fig:FRK_AIRS_ST2}",fig.align="center",out.width="\\linewidth",fig.height=3,fig.width=16,fig.pos="t!">>=
...
<<echo=FALSE, fig.keep=TRUE,fig.cap="Prediction standard error of $\\Yvec_P$ in ppm on 04, 08 and 12 May 2003 obtained with \\tt{FRK} on the \\tt{AIRS} data. \\label{fig:FRK_AIRS_ST3}",fig.align="center",out.width="\\linewidth",fig.height=3,fig.width=16,fig.pos="t!">>=

X$se <- sqrt(X$var)
g3 <-  (EmptyTheme() +
            geom_polygon(data=filter(X,(t %in% c(4,8,12)) & abs(lon) < 175),
                         aes(lon,lat,fill=pmin(se,2.9),group=id))+
           scale_fill_distiller(palette="Spectral",name="s.e. (ppm)") +
           coord_map("mollweide") +
           facet_grid(~t) +
            xlab("lon (deg)") +
...
g2 <-LinePlotTheme() +
     scale_fill_distiller(palette="Spectral",name ="s.e.") +
...
\end{itemize}
There is no constructor yet for \tt{Basis}, and the \tt{R} command \tt{new} needs to be used to create this object from scratch.
...
Package devlopment was facilitated with \tt{devtools}; this vignette was created with \tt{knitr}, and package testing was carried out using \tt{testthat} and \tt{covr}. The package includes within it some date manipulation functions from \tt{Hmisc}, and the function \tt{rdist.earth} from the package \tt{fields}. Some sparse-matrix operations are facilitated using \tt{C} code from the software package \tt{SuiteSparse} \citep{SuiteSparse}. We thank Jonathan Bradley and Chris Wikle for helpful discussions on Fixed Rank Kriging.
