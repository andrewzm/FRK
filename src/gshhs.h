 ...
 * -----------------------------------------------------------------------------
 * $Id: gshhs.h 9656 2012-01-13 18:29:02Z pwessel $
 *
...
/* callbacks used when extracting data; the original gshhs.c prints the 
   following fields to stdout; the modified version calls the functions
   below rather than printing */
extern void (*polygonHeader) (char c, int  h_id, int h_n, int level, 
			      char source, double area, double f_area,
			      double w, double e, double s, double n,
			      int container, int ancestor);
extern void (*lineHeader) (char c, int h_id, int h_n, int level, char source,
			   double w, double e, double s, double n);
extern void (*point) (double lon, double lat);
#endif /* STANDALONE */
