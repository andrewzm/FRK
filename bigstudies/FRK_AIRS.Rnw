...
## Load data
data(AIRS_05_2003)
#AIRS_05_2003$co2std <- sqrt(5.6)
AIRS_05_2003 <- filter(AIRS_05_2003,day %in% 1:5) %>%
    mutate(std=co2std) %>%
...
<<eval=TRUE>>=
## Prediction (BAU) grid
isea3h_sp_poldf <- auto_BAUs(manifold=sphere(),
                             res=7,
                             data=NULL,
                             cellsize = c(5,5),
                             type="hex")
...
<<eval=TRUE>>=
S <- SRE.fit(S,n_EM = 200,tol = 1e-5,print_lik=TRUE)
@
