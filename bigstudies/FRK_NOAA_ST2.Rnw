...
Tmax <- subset(NOAA_df_1990,    # subset the data
              proc=="Tmax") %>% # Only max. temperatures
        group_by(year,id) %>%   # group by year and station
        summarise(z = mean(z),lon=lon[1],lat=lat[1]) %>%   # find average at each station
        mutate(time = as.Date(paste(year,06,01,sep="-"))) %>%   # create date object
        mutate(std = 0.001) %>%     # Set measurement error to some value (this needs to be known)
        dplyr::select(z,lon,lat,time,id,std) %>%  # select subset of columns
        as.data.frame()         # convert to data frame

STObj <- stConstruct(x = Tmax,                  # dataset
                 space = c("lon","lat"),        # spatial fields
                 time="time",                   # time field
                 crs=CRS("+proj=longlat +ellps=sphere"),      # projection (lon-lat)
                 interval=TRUE)                 # time is an interval

## Remove projection information
proj4string(STObj) <- ""                        # we are not working on the sphere, use directly the lon-lat coordinates (not recommended, use spTransform instead)
...
grid_BAUs <- SRE.predict(S,pred_locs = grid_BAUs,use_centroid = TRUE,include_fs=FALSE)
